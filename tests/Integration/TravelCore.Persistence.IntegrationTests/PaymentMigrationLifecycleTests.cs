...
public sealed class PaymentMigrationLifecycleTests
{
    ...
    public async Task PaymentMigrationLifecycle_Apply_EnsureSchema_Only()
    {
        ...
        await using (var inventoryDb = _postgres.CreateDbContext())
        {
            ...
            Assert.EndsWith("_AddFlightBookingCancellationRefundInbox", expectedMigrations[10], StringComparison.Ordinal);
        }
        ...
        await using (var db = _postgres.CreateDbContext())
        {
            ...
            Assert.Equal(1, await ScalarIntAsync(conn, """
                ...
                """, ct));
            Assert.Equal(11, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
                FROM information_schema.tables
                WHERE table_schema = 'payment'
            ...
            Assert.Equal(1, await ScalarIntAsync(conn, """
                ...
                FROM information_schema.tables
                WHERE table_schema = 'payment'
                  AND table_name = 'flight_booking_cancellation_refund_inbox';
                """, ct));
            Assert.Equal(0, await ScalarIntAsync(conn, """
                ...
                WHERE table_schema = 'payment'
                  AND table_name IN (
                        ...
                        'provider_callbacks', 'settlements', 'wallets');
                """, ct));
            Assert.Equal(0, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
