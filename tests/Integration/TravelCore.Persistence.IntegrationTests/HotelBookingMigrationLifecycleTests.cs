    ...
    public async Task HotelBookingMigrationLifecycle_Apply_Stay_Structure()
    {
        ...
        await using (var db = _postgres.CreateDbContext())
        {
            ...
            Assert.Equal(1, await ScalarIntAsync(conn, """
                ...
                FROM information_schema.tables
                WHERE table_schema = 'hotel_booking'
                  AND table_name = 'hotel_supplier_reservation_attempts';
                """, ct));
            Assert.Equal(1, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
                FROM information_schema.tables
                WHERE table_schema = 'hotel_booking'
                  AND table_name = 'hotel_supplier_reservation_idempotency';
                """, ct));
            Assert.Equal(1, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
                FROM information_schema.tables
                WHERE table_schema = 'hotel_booking'
                  AND table_name = 'hotel_booking_reconciliation_issues';
                """, ct));
            Assert.Equal(1, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
                FROM information_schema.tables
                WHERE table_schema = 'hotel_booking'
                  AND table_name = 'hotel_booking_payment_evidence';
                """, ct));
            Assert.Equal(1, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
                FROM information_schema.tables
                WHERE table_schema = 'hotel_booking'
                  AND table_name = 'payment_success_inbox';
                """, ct));
            Assert.Equal(1, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
                FROM information_schema.tables
                WHERE table_schema = 'hotel_booking'
                  AND table_name = 'refund_success_inbox';
                """, ct));
            Assert.Equal(1, await ScalarIntAsync(conn, """
                SELECT COUNT(*)::int
                FROM information_schema.tables
