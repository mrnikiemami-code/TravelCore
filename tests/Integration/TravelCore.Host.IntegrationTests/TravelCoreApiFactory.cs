...
public sealed class IdentityAuthHostFixture : IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        await using var tripPlanner = CreateTripPlannerDb();
        await TripPlannerMigrator.MigrateAsync(tripPlanner);

        await using var booking = CreateBookingDb();
        await BookingMigrator.MigrateAsync(booking);

        await using var payment = CreatePaymentDb();
        await PaymentMigrator.MigrateAsync(payment);
    ...
    public TripPlannerDbContext CreateTripPlannerDb()
    {
        var options = new Microsoft.EntityFrameworkCore.DbContextOptionsBuilder<TripPlannerDbContext>()
    ...
    public BookingDbContext CreateBookingDb()
    {
        var options = new Microsoft.EntityFrameworkCore.DbContextOptionsBuilder<BookingDbContext>()
            .UseTravelCorePostgreSql(ConnectionString, migrationsHistorySchema: BookingDbContext.SchemaName)
            .Options;
        return new BookingDbContext(options);
    }

    public PaymentDbContext CreatePaymentDb()
    {
        var options = new Microsoft.EntityFrameworkCore.DbContextOptionsBuilder<PaymentDbContext>()
            .UseTravelCorePostgreSql(ConnectionString, migrationsHistorySchema: PaymentDbContext.SchemaName)
            .Options;
        return new PaymentDbContext(options);
    }

    public HotelBookingDbContext CreateHotelBookingDb()
    {
        var options = new Microsoft.EntityFrameworkCore.DbContextOptionsBuilder<HotelBookingDbContext>()
