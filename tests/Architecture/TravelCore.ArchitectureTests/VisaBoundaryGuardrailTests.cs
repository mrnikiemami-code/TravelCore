...
/// <summary>
/// TC-P17-T001: Visa independent module + schema visa. No VisaDefinition/requirement product types.
...
public sealed class VisaBoundaryGuardrailTests
{
    ...
    public void Visa_DoesNot_Own_Peer_SoT_Or_Product_Types()
    {
        ...
        Assert.False(VisaOwnershipBoundary.VisaDefinitionImplemented);
    ...
    [Fact]
    public void Visa_T001_MustNotImplement_ProductAggregates()
    {
        var roots = new[]
        {
        ...
        var forbiddenType = new Regex(
            @"\b(class|record|enum|struct|interface)\s+(VisaDefinition|VisaRequirement|RequiredDocument|EligibilityRule|VisaFee|VisaApplication|VisaOffering|Country|Destination|Nationality|Region)\b",
        ...
        Assert.True(
            hits.Count == 0,
            "T001 forbids VisaDefinition/requirement/document/fee/application and geo-clone product types:\n" + string.Join('\n', hits));
    }
