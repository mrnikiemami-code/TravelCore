...
| Search | Platform | Discovery/retrieval + hybrid read-model (`SearchDocument` / `ISearchIndex`); schema `search` | Events/contracts from publishers (later) | Authoritative Tour/Content/Pricing/Agency/Destination/Place/UGC/Visa facts; SEO IndexPolicy; Booking; Recommendation | Being required by core business transactions; no FTS/ranking/faceting engine in T001–T002 |
