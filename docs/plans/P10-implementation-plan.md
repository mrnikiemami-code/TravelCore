...
| **P10-R5** | Meals model shape | **UNRESOLVED** | Per-day vs per-stop vs itinerary-level meals; enum vs localized text; relation to glossary MealPlan (package-oriented) without importing P11 product. |
| **P10-R6** | Difficulty / Eligibility taxonomy | **UNRESOLVED** | Closed enum vs ReferenceData vs free-form localized text; eligibility rules as product facts only (not Booking engine). |
