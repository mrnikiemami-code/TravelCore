...
| **P07-R3** | Place delete/archive lifecycle | **UNRESOLVED** | Do not invent hard-delete product; archive/status may suffice — architect lock. **T004 note:** `PlaceCatalogStatus` {Draft, Active, Inactive} is catalog operational status only — it is **not** Deleted/SoftDeleted/Archived/Retired and does **not** resolve this decision. |
| **P07-R4** | Slug ownership (Place-localized slug vs SEO-only route key) | **RESOLVED** | **PLACE owns current locale-specific Slug** on `PlaceTranslation` (Locale, Name, Description, Slug). **SEO owns** route binding, publication namespace, reservations, historical paths, redirect history, canonical/publication mechanics, IndexPolicy. Place current slug ≠ SEO route history ≠ redirect history. Changing Place slug updates Place current truth; redirects/history stay SEO-owned. No Place redirect-history fields; no SEO ownership of `PlaceTranslation.Slug`; no global `Place.Slug` / SlugFa/SlugEn (ADR 0008). Normalization mirrors Destination/P04. |
| **P07-R5** | Public IndexPolicy default for new Place | **RESOLVED** | Default Place SEO posture: **NOINDEX, FOLLOW**. Public ≠ Index; Active ≠ Index; Publish ≠ Index. Missing/default IndexPolicy ⇒ `noindex, follow`. Do **not** auto-index because Active / public route / translation / Cover / Destination. Do **not** inherit Destination IndexPolicy. Index requires explicit SEO authority/policy. |
