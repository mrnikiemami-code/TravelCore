...
|----|-------|--------|-------|
| **P07-R1** | Place model shape (single Place + kind vs typed Hotel/Restaurant/Attraction aggregates / TPH) | **UNRESOLVED** | Must resolve before/at T002. STOP if envelope requires lock and missing. |
| **P07-R2** | Destination link requiredness/cardinality (every Place must have DestinationId?) | **UNRESOLVED** | Affects T003 validation. Prefer reference-by-id; no cross-schema FK. |
