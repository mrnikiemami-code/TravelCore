...
| **P21-R8** | Public UX / anonymous-auth / privacy / operational reads / supplier-provider readiness | **RESOLVED** — public HotelBooking is transactional and behavior-oriented, not CRUD · anonymous HotelBooking uses a HotelBooking-specific opaque access token (`X-TravelCore-Hotel-Booking-Access-Token`) independent of Tour `X-TravelCore-Booking-Access-Token` · raw token returned once; SHA-256 verifier/hash persisted; raw token never enters URL · HotelBookingId/PaymentId/SupplierReservationId are not credentials · authenticated callers require object-level authorization · initiation is DB-idempotent · customer amount/currency/success are never authoritative · availability/rate/reservation sources remain server-controlled · zero production Hotel sources is a valid host posture · no fake production availability/rate/reservation truth · HotelBooking Payment access is HotelBooking-scoped · Payment provider selection remains server-controlled · no raw card collection · private HotelBooking/payment pages are noindex · Payment Succeeded / HotelBooking Pending is a first-class customer state · confirmed cancellation API uses R7 policy and supplier authority · partial-refund-required cancellation remains blocked · operational HotelBooking reads are read-only · no manual financial/booking truth mutation exists · Hotel source/provider capabilities, if modeled, are explicit · no smart routing/failover · Named Hotel Supplier = NONE · Production Payment Provider = NONE · Partial Refund / amendments / PayAtProperty / deposits remain DEFERRED |
...
- Depends on **P21-R3**. **COMPLETE / ACCEPTED** (`2696407` / docs `77a9b8f` / VERIFY `5824acd` / `14c594c`)
...
- Depends on **P21-R6**. **COMPLETE / ACCEPTED** (`f2d4946` / docs `790765b`) — typed HotelBooking Payment target (nullable `booking_id` + `hotel_booking_id`, exactly-one CHECK, filtered uniques) · pay-first supplier reservation · dual-evidence confirmation · Hotel-specific outbox/inbox events · full Refund compensation via existing P20 Refund · no public HotelBooking Payment API · Production Payment Provider = NONE.

### TC-P21-T007 — Cancellation / amendment / refund-policy boundary

- Depends on **P21-R7**. **COMPLETE / ACCEPTED** (`c3fabe9` / docs `836cd92`) — Confirmed HotelBooking cancellation via `HotelBookingCancellation` (Requested / SupplierCancellationPending / RefundPending / Completed) · immutable policy evaluation at RequestedAt · `HotelSupplierCancellationAttempt` Created/Initiated/Confirmed/Failed · Penalty=0 full Refund after authoritative supplier cancel · Penalty=Total completes without Refund · partial penalty blocked before supplier side effect · Partial Refund DEFERRED · amendments/rebooking DEFERRED · PayAtProperty/deposit DEFERRED · Named Hotel Supplier = NONE · Production Payment Provider = NONE.
...
- Guardrails + evidence pack [`P21-T009-hardening-and-evidence-pack.md`](P21-T009-hardening-and-evidence-pack.md); **no new capability**. **COMPLETE / ACCEPTED** (`ae84f62` / docs `2706bfb`).
