...
| Status | PLAN ACCEPTED; **P19-R1–R8 RESOLVED**; T001–T009 ACCEPTED; **P19 COMPLETE** (`TC-P19-GATE`) |
...
- Purpose: Historical commercial facts inside Booking (**P19-R5 RESOLVED**).
- Delivered: Booking consumes a valid authoritative Pricing Quote via `IAuthoritativeQuoteQuery`; stores immutable `BookingMonetarySnapshot` + structured components; logical `PricingQuoteReference`; `AcceptQuote` is idempotent for the same Quote and rejects a different Quote, expired Quote, and TourDeparture mismatch. No recalculation, FX, Payment, or confirmation.
- Preserve: **Price != Quote** · **Quote != BookingMonetarySnapshot** · **BookingMonetarySnapshot != PaymentAmount** · **Booking != Pricing Authority** · **QuoteExpired != BookingStatus** · **QuoteExpiresAt != CapacityHold.ExpiresAt** · **BudgetPreference != BookingMonetarySnapshot**. Quote engine stays Pricing. Live Price must not rewrite accepted Booking snapshot.

### TC-P19-T006 — Confirmation / cancellation / Payment interaction contracts

- Purpose: Confirmation and cancellation ownership; Payment **orchestration boundary** (**P19-R6 RESOLVED**).
