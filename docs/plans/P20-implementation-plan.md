...
| Plan-ID | `TC-P20-PLAN` |
| Phase | P20 — Payment |
| Status | COMPLETE · GATE ACCEPTED · **P20-R1 = RESOLVED** · **P20-R2 = RESOLVED** · **P20-R3 = RESOLVED** · **P20-R4 = RESOLVED** · **P20-R5 = RESOLVED** · **P20-R6 = RESOLVED** · **P20-R7 = RESOLVED** · **P20-R8 = RESOLVED** · T001–T009 ACCEPTED |
...
| **P20-R5** | Booking confirmation integration after authoritative Payment success | **RESOLVED** | Payment binds to immutable Booking monetary obligation via `PaymentExecutionSnapshot`, enforces provider amount/currency integrity, and uses Booking-owned confirmation transaction (consume Active CapacityHold, Pending -> Confirmed). Payment must not write Booking tables. PaymentSucceeded remains distinct from BookingConfirmed; when confirmation prerequisites fail, Booking records recovery evidence and emits compensation-required (T006). |
| **P20-R6** | Refund / cancellation / compensation boundary | **RESOLVED** | Successful Payment + failed Booking confirmation requires a Payment-owned full Refund. **Payment != Refund**. **PaymentSucceeded != RefundSucceeded**. **RefundSucceeded != BookingCancelled**. PaymentStatus remains Succeeded after refund. RefundStatus = Pending / Succeeded. One full Refund per Succeeded Payment; amount/currency from `PaymentExecutionSnapshot` only. Booking recovery writes a Booking-local compensation-required outbox atomically. Payment GetOrCreate Refund does not trust event amount. Authoritative refund success writes RefundSucceeded outbox. Booking cancels Pending and releases Active hold; Confirmed cancel and Consumed reversal remain deferred. No public refund API. No partial refund. Named provider remains NONE. |
| **P20-R7** | Public Payment UX / anonymous-authenticated authorization / privacy | **RESOLVED** | Public Payment is Booking-scoped, not standalone CRUD. Booking object authorization controls Payment access. Anonymous Booking reuses accepted `X-TravelCore-Booking-Access-Token`. BookingId/PaymentId are not credentials. Authenticated access requires object-level ownership. Public initiation never accepts authoritative amount/currency/success. Provider selection is server-controlled. Browser return is navigation/status only. Provider callback remains the verification boundary. No raw card collection. No public Refund API. Private Payment/Booking pages are noindex. Payment Succeeded while Booking confirmation is pending is a first-class UX state. Mobile-first, accessible, FA/EN/AR, bidi-safe, Server Component First. No real provider selected. Provider configuration/capabilities remain P20-R8. |
...
- Guardrails + evidence pack; **no new capability**. Evidence: [`P20-T009-hardening-and-evidence-pack.md`](P20-T009-hardening-and-evidence-pack.md). **ACCEPTED** `75456e9` / docs `e5ba5e6`.

### TC-P20-GATE — Acceptance Gate

- Evidence only. No new Payment product in GATE. Do not start P21 inside GATE. Evidence: [`P20-GATE-acceptance-evidence.md`](P20-GATE-acceptance-evidence.md).
