...
/**
 * Public Article detail (TC-P08-T008).
...
export async function generateMetadata({
  ...
  const loaded = await loadContentDetailPage(localeParam, slug, "Article");
  if (!isApiOk(loaded)) {
    return { title: "TravelCore", robots: { index: false, follow: false } };
  }
...
export default async function ArticleDetailPage({ params }: PageProps) {
  ...
  const loaded = await loadContentDetailPage(locale, slug, "Article");
  if (!isApiOk(loaded)) {
    notFound();
  }
  ...
  return (
    <PublicShell
      ...
      footer={
        <Text role="caption">
          {locale === "fa"
            ? "P08 — مقاله عمومی · SEO metadata"
            : "P08 — public Article · SEO metadata"}
        </Text>
      }
    >
      {breadcrumbScript ? (
