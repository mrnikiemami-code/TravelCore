...
/// <summary>
/// FlightBooking-scoped public Payment status and initiation (TC-P22-T008 / P22-R8).
/// Does not accept client amount/currency/success. Does not fabricate a provider redirect.
/// </summary>
internal sealed class PublicFlightBookingPaymentService : IPublicFlightBookingPaymentService
{
    ...
    {
        var payment = await LoadOrCreateAsync(flightBookingId, cancellationToken);
        await TryPrepareAsync(payment.Id, cancellationToken);
        return await MapAsync(payment.Id, redirectUri: null, cancellationToken);
    }

    public async Task<PublicPaymentCommandResult> InitiateForFlightBookingAsync(
        Guid flightBookingId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await _initiation.InitiateForFlightBookingAsync(
            new FlightBookingPaymentReference(flightBookingId),
            idempotencyKey,
            cancellationToken);
        return new PublicPaymentCommandResult(
    ...
    private async Task<PaymentAggregate> LoadOrCreateAsync(Guid flightBookingId, CancellationToken cancellationToken)
    {
