...
/// <summary>
/// HotelBooking-scoped public Payment status and initiation (TC-P21-T008 / P21-R8).
/// Does not accept client amount/currency/success. Does not fabricate a provider redirect.
/// </summary>
internal sealed class PublicHotelBookingPaymentService : IPublicHotelBookingPaymentService
{
    ...
    public PublicHotelBookingPaymentService(
        PaymentDbContext db,
    ...
    public async Task<PublicPaymentRead> GetByHotelBookingIdAsync(
        Guid hotelBookingId,
        CancellationToken cancellationToken = default)
    {
        var payment = await LoadOrCreateAsync(hotelBookingId, cancellationToken);
        await TryPrepareAsync(payment.Id, cancellationToken);
        return await MapAsync(payment.Id, redirectUri: null, cancellationToken);
    }

    public async Task<PublicPaymentCommandResult> InitiateForHotelBookingAsync(
        Guid hotelBookingId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await _initiation.InitiateForHotelBookingAsync(
            new HotelBookingPaymentReference(hotelBookingId),
            idempotencyKey,
            cancellationToken);
        return new PublicPaymentCommandResult(
    ...
    private async Task<PaymentAggregate> LoadOrCreateAsync(Guid hotelBookingId, CancellationToken cancellationToken)
    {
        var created = await _getOrCreate.GetOrCreateAsync(
            new HotelBookingPaymentReference(hotelBookingId),
            cancellationToken);
