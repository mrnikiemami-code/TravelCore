...
- **Booking:** Booking module scaffolding (`TC-P19-T001`) + aggregate/lifecycle (`TC-P19-T002`) + capacity hold (`TC-P19-T003`) + booker/passengers (`TC-P19-T004`) + Quote snapshot (`TC-P19-T005`) + Pending cancellation orchestration (`TC-P19-T006`) + Direct/Agency source context (`TC-P19-T007`) + public Pending initiation / authorization / privacy (`TC-P19-T008`) + hardening/evidence (`TC-P19-T009`; GATE NOT EXECUTED) — schema `booking`; tables `bookings`, `capacity_holds`, `departure_capacity_accounts`, `booking_passengers`, `booking_monetary_snapshots`, `booking_monetary_snapshot_components`, `booking_access_credentials`, `booking_public_idempotency`; initial logical target = TourDeparture; **Booking != Tour**; **CapacityDefinition != CapacityConsumption**; statuses Pending/Confirmed/Cancelled; hold states Active/Consumed/Released/Expired; **CapacityHoldStatus != BookingStatus**; **Pending != CapacityHeld**; **PlannerTravelerComposition != BookingPassenger**; **BookingPassenger != Party Person Master**; **Price != Quote**; **Quote != BookingMonetarySnapshot**; **BookingMonetarySnapshot != PaymentAmount**; **Booking != Payment**; **PaymentSucceeded != BookingConfirmed**; **Booking != AgencyMarketplace**; **BookingSourceKind != BookingStatus**; **AgencyOffer != Booking**; **AgencyOffer != Quote**; **Agency context != Pricing Authority**; **PublicExperience != Booking Source of Truth**; **Public Booking initiation != Booking confirmation**; **Pending != Confirmed**; **BookingId != Access Credential**; Tour owns capacity definition; Booking owns consumption via temporary hold; Booking copies a valid Pricing Quote into an immutable snapshot; Pending cancel atomically releases Active holds; Direct and Agency share one Booking aggregate with logical AgencyProfile/Offer refs; public Direct initiation issues a Quote, holds capacity, and returns a hashed-token credential once; Confirm/Payment DEFERRED; no passport/public listing; no peer FK.
- **P19-R1 RESOLVED:** independent Booking module with schema `booking`. Owns transactional Tour booking facts and capacity consumption. Does not own TourProduct/TourDeparture catalog, capacity definition, Pricing/Quote, Payment, Party/Identity master, AgencyMarketplace, Search, SEO, Notification delivery, VisaApplication, or TripPlanner Lead. Product references are opaque logical ids only (`TourDepartureReference`).
...
- **P19-R3 RESOLVED:** Booking-owned `CapacityHold` + per-departure consumption account. Hold states Active/Consumed/Released/Expired. Explicit `ExpiresAt`. Atomic overbooking protection via PostgreSQL advisory transaction lock. Idempotent hold. Confirmation remains R6.
- **P19-R4 RESOLVED:** BookingContactSnapshot is transaction-time contact data. BookingPassenger is a Booking-owned child. **PlannerTravelerComposition != BookingPassenger**. **BookingPassenger != Party Person Master**. No passport/document upload. PassengerCount cannot exceed Active hold SeatCount.
- **P19-R5 RESOLVED:** Pricing owns Price and Quote. Booking consumes a valid authoritative Quote and stores immutable `BookingMonetarySnapshot`. **Price != Quote**. **Quote != BookingMonetarySnapshot**. **BookingMonetarySnapshot != PaymentAmount**. **Booking != Pricing Authority**. No FX, tax/discount recalculation, Payment, or confirmation.
- **P19-R6 RESOLVED:** Payment execution is outside P19. Executable Confirm = **DEFERRED to Payment integration**. Pending cancellation is IN and atomically releases Active holds. Confirmed cancellation/refund = **DEFERRED**. **Booking != Payment**. **PaymentSucceeded != BookingConfirmed**. **BookingCancelled != PaymentRefunded**.
- **P19-R7 RESOLVED:** Direct and Agency-originated bookings use the same Booking aggregate. `BookingSourceKind` Direct/Agency. Agency source requires logical AgencyProfileReference; AgencyOfferReference is optional. **Booking != AgencyMarketplace**. **BookingSourceKind != BookingStatus**. **AgencyOffer != Booking**. **AgencyOffer != Quote**. **Agency context != Pricing Authority**. No commission/settlement/agency price/acceptance/capacity pool. Agency PII/authorization is object-level (R8).
- **P19-R8 RESOLVED:** PublicExperience composes honest Prepare-booking CTA. Booking remains SoT. Public initiation creates Pending Booking only. Opaque hashed Booking-scoped access token; authenticated object-level actor reads. Direct public path; Agency origin cannot be client-forged. Transaction pages noindex. No Confirm/Payment/listing/public cancellation. **PublicExperience != Booking Source of Truth**. **Public Booking initiation != Booking confirmation**. **Pending != Confirmed**. **BookingId != Access Credential**.
