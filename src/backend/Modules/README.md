...
- **Visa:** VisaDefinition + RequirementSet + Applicability + RequiredDocument + EligibilityRequirement + ProcessingTime/Validity/AllowedStay/EntryPolicy (`TC-P17-T005`) — schema `visa`; **ProcessingTime != VisaValidity**; **VisaValidity != AllowedStay**; no fees/application/upload tables; no peer FK.
