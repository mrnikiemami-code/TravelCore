...
- **Booking:** Booking module scaffolding (`TC-P19-T001`) + aggregate/lifecycle (`TC-P19-T002`) + capacity hold (`TC-P19-T003`) — schema `booking`; tables `bookings`, `capacity_holds`, `departure_capacity_accounts`; initial logical target = TourDeparture; **Booking != Tour**; **CapacityDefinition != CapacityConsumption**; statuses Pending/Confirmed/Cancelled; hold states Active/Consumed/Released/Expired; **CapacityHoldStatus != BookingStatus**; **Pending != CapacityHeld**; Tour owns capacity definition; Booking owns consumption via temporary hold; no passenger/public types; no peer FK.
