...
- **Booking:** Booking module scaffolding (`TC-P19-T001`) + aggregate/lifecycle (`TC-P19-T002`) + capacity hold (`TC-P19-T003`) + booker/passengers (`TC-P19-T004`) + Quote snapshot (`TC-P19-T005`) — schema `booking`; tables `bookings`, `capacity_holds`, `departure_capacity_accounts`, `booking_passengers`, `booking_monetary_snapshots`, `booking_monetary_snapshot_components`; initial logical target = TourDeparture; **Booking != Tour**; **CapacityDefinition != CapacityConsumption**; statuses Pending/Confirmed/Cancelled; hold states Active/Consumed/Released/Expired; **CapacityHoldStatus != BookingStatus**; **Pending != CapacityHeld**; **PlannerTravelerComposition != BookingPassenger**; **BookingPassenger != Party Person Master**; **Price != Quote**; **Quote != BookingMonetarySnapshot**; **BookingMonetarySnapshot != PaymentAmount**; Tour owns capacity definition; Booking owns consumption via temporary hold; Booking copies a valid Pricing Quote into an immutable snapshot; no passport/public types; no peer FK.
