...
public sealed class HotelBookingCancellationRefundOutboxDispatcher
{
    private readonly HotelBookingDbContext _db;
    private readonly IHotelBookingCancellationRefundRequiredHandler _handler;
    private readonly IClock _clock;

    public HotelBookingCancellationRefundOutboxDispatcher(
        HotelBookingDbContext db,
        IHotelBookingCancellationRefundRequiredHandler handler,
        IClock clock)
    {
        _db = db;
        _handler = handler;
    ...
    public async Task<int> DispatchPendingAsync(int take = 50, CancellationToken cancellationToken = default)
    {
        ...
        {
            var evt = HotelBookingCancellationRefundOutboxSerializer.Deserialize(message.Payload);
